...
class UNetModel_newpreview(nn.Module):
    ...
    def __init__(
        ...
        self.num_head_channels = num_head_channels
        self.num_heads_upsample = num_heads_upsample
        self.unet_seg_model = unet_seg_model
        ...
        if high_way:
            features = 32
            self.hwm = Generic_UNet(self.in_channels, features, 1, 5, anchor_out=True, upscale_logits=True)
    ...
    def forward(self, x, text, timesteps, y=None):
        """
        Apply the model to an input batch.
        ...
        hs = []
      #   #print(self.unet_seg_model)
      #   #unet_output = self.unet_seg_model(x)
      #   #print()
        
      #   # unet_output = torch.randn_like(x)
      #   print("******** Size of x ************")
      #   print(x.size())
      #  # X_numpy = x[0].permute(1, 2, 0).detach().cpu().numpy()
      #  # plt.imsave('X_image.png', X_numpy)
      #   print("******** Size of unet_outtput ************")
      #   #print( unet_output.size())
        # text = clip.tokenize(text).to(torch.device('cuda')) # device represent here cpu or gpu
        tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-base-patch32")
        inputs = tokenizer(text, padding=True, return_tensors="pt").to("cuda")
        text_features = self.text_model(**inputs).pooler_output
        ...
        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)
            emb = emb + self.label_emb(y)
        emb = emb + text_features
        # print("**** Size of x after if condition *********")
        # print(x.size())
        h = x.type(self.dtype)
        X_numpy = h[:,2:,...][0][0].detach().cpu().numpy()
        #plt.imsave('X_image1.png', X_numpy)
        # print("******** Size of h ************")
        # print(h.size())
        c = h[:,:-1,...]
        unet_output = self.unet_seg_model(c)
        # print("********** This is c at first **********")
        # print(c.size())
        C_numpy = c[0].permute(1, 2, 0).detach().cpu().numpy()
        # plt.imshow(C_numpy)
        #plt.imsave('C_image.png', C_numpy)
        # plt.imshow(c[0].permute(1,2,0). detach ().cpu().numpy ())
        # plt.show()
        # #unet_output = unet_output[:,2:,...]
        # print("******** Size of unet_output  after operation ************")
        # print( unet_output.size())
        #plt.imsave('unet_output', unet_output)


        if unet_output is not None:
            unet_output = unet_output.type(self.dtype)
            c = torch.cat([c, unet_output], dim=1)
        # print("*****c after concatenation*******")
        # print(c.size())

        #c = c[:,:-2,...]
        # print("*****c after reduction*******")
        #print(c.size())
        ...
        for ind, module in enumerate(self.input_blocks):
            if len(emb.size()) > 2:
                # print("The first if is running")
                emb = emb.squeeze()
            if ind == 0:
                # print("The second if is running")
                h = module(h, emb)
            ...
            hs.append(h)
        h = self.middle_block(h, emb)
        # text should contains the list of text, the length of text equals number of batch
        # text = clip.tokenize(text).to(torch.device('cuda')) # device represent here cpu or gpu
        # text_features = self.text_model.encode_text(text) # you should initialize the model same as unet_seg_model
        # print("|||||||||||||||||||||||||||||||||||||||||||||||||||")
        # text_features_ = text_features.unsqueeze(2).unsqueeze(3)
        # text_features_ = text_features_.expand(-1, -1, h.size(2), h.size(3))
        # print(text_features_.size())
        # print("||||||||||||||||||||||||||||||||||||||||||||||||||||")
        for module in self.output_blocks:
            hs_ = hs.pop()
            # print("===============================|||||||=================================")
            # print(h.size(), " ", hs_.size(), " ", text_features_.size())
            # print("================================|||||||======================================")
            h = th.cat([h, hs_], dim=1) # here you might face the embedding shape issue, you have to fix it.
            h = module(h, emb)
        h = h.type(x.dtype)
        out = self.out(h)
        return out, cal
...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for u in range(num_pool):
            ...
            # print("*** The nnfeatures******")
            # print(nfeatures_from_down)
            # the first conv reduces the number of features to match those of skip
            # the following convs work on that number of features
            # if not convolutional upsampling then the final conv reduces the num of features again
            if u != num_pool - 1 and not self.convolutional_upsampling:
                final_num_features = self.conv_blocks_context[-(3 + u)].output_channels
                # print("*** The final_num_features in if******")
                # print(final_num_features)
            else:
                final_num_features = nfeatures_from_skip
                # print("*** The final_num_features in else******")
                # print(final_num_features)
    ...
    def forward(self, x, hs = None):
        ...
        for d in range(len(self.conv_blocks_context) - 1):
            ...
            skips.append(x)
            # print("******x in generic function outside if*******")
            # print(x.size())
            if not self.convolutional_pooling:
                x = self.td[d](x)
                # print("******x in generic function inside first if*******")
                # print(x.size())
            if hs:
                h = hs.pop(0)
                ddims = h.size(1)
                ...
                hb = hb[:,:,None,None]
                x = x * ha * hb
                # print("******x in generic function inside second if*******")
                # print(x.size())
